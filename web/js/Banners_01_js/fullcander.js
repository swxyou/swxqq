...
ignoreWindowResize--}}
function renderView(inc){if(elementVisible()){var currentTime=new Date();var currentYear=currentTime.getFullYear();var currentMoth=hasPriceMonth;var DDYear=date.getFullYear(),DDMoth=date.getMonth();var preDisable=false,nextDisable=false;$('.fc-button.fc-button-prev').removeClass('disable');$('.fc-button.fc-button-next').removeClass('disable');if(preDisable){$('.fc-button.fc-button-prev').removeClass('disable').addClass('disable');}
if(nextDisable){$('.fc-button.fc-button-next').removeClass('disable').addClass('disable');}
if(inc==-1&&preDisable){return;}
if(inc==1&&nextDisable){return;}
ignoreWindowResize++;unselect();if(suggestedViewHeight===undefined){calcSize()}
...
function hpadding(element){return(parseFloat(element.css('paddingLeft',true))||0)+(parseFloat(element.css('paddingRight',true))||0)}
function hmargins(element){return(parseFloat(element.css('marginLeft',true))||0)+(parseFloat(element.css('marginRight',true))||0)}
function hborders(element){return(parseFloat(element.css('borderLeftWidth',true))||0)+(parseFloat(element.css('borderRightWidth',true))||0)}
function vsides(element,includeMargins){return vpadding(element)+vborders(element)+(includeMargins?vmargins(element):0)}
function vpadding(element){return(parseFloat(element.css('paddingTop',true))||0)+(parseFloat(element.css('paddingBottom',true))||0)}
function vmargins(element){}
function vborders(element){return(parseFloat(element.css('borderTopWidth',true))||0)+(parseFloat(element.css('borderBottomWidth',true))||0)}
function setMinHeight(element,height){height=(typeof height=='number'?height+'px':height);element.each(function(i,_element){_element.style.cssText+=';min-height:72px;_height:'+height})}
function noop(){}
...
html+="<span class='fc-event-title "+htmlEscape(lastSpanCss+" "+iconCss)+"'>"+htmlEscape(event.title)+"</span></div>"}else{html+="<span class='fc-event-title "+htmlEscape(event._id)+"'>"+htmlEscape(event.title)+"</span></div>";}
if(seg.isEnd&&isEventResizable(event)){html+="<div class='ui-resizable-handle ui-resizable-"+(rtl?'w':'e')+"'>&nbsp;&nbsp;&nbsp;</div>"}
